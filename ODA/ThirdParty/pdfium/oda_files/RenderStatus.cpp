...
bool OdRenderStatus::ProcessText(CPDF_TextObject* textobj, const CFX_Matrix* pObj2Device, CFX_PathData* pClippingPath)
{
  ...
  if(CPDF_Font::kInvalidCharCode != space_char_code)
    space_width = static_cast<float>(pFont->GetCharWidthF(space_char_code))* fontsize / 1000.;
  if (space_width < 1e-7)
  {
    for (auto& elem : textobj->GetCharCodes())
    {
      ...
      {
        const float val = static_cast<float>(pFont->GetCharWidthF(elem)* fontsize / 1000.);
        if (space_width < 1e-7)
        {
          space_width = val;
        }
  ...
  for (auto i = 0; i < textobj->GetCharCodes().size(); ++i)
  {
    ...
    if (CPDF_Font::kInvalidCharCode == charcode || charcode == space_char_code)
    {
        ...
        (CPDF_Font::kInvalidCharCode != textobj->GetCharCodes()[next_idx] && textobj->GetCharCodes()[next_idx] != space_char_code))
      {
        ...
        auto next_pos = textobj->GetCharPositions()[i];
        ...
        if (isVertWriting)
        {
          ...
          last_symbol_width = static_cast<float>(pFont->GetCharWidthF(textobj->GetCharCodes()[cur_idx]))* fontsize / 1000.; 
        auto last_symbol_end = last_pos + last_symbol_width + char_space;
        if ((last_pos > next_pos) || (last_symbol_end + 1.5 * space_width < next_pos))
        {
          if (start_idx > 0)
          {
            auto j = start_idx - 1;
        ...
        if(next_pos - last_symbol_end < 0.5 * space_width)
          continue;
        if (codes_storage.back().size() > 0)
  ...
  for (auto i = 0; i < codes_storage.size(); ++i)
  {
    ...
    const bool invalid_as_geometry = getOption(m_PdfImportOptions, kImportInvalidFontAsGeometry) && textobj->IsHexStringSource();
    bool is_vaid_str = false;
    for (auto& elem : codes_storage[i])
    {
      WideString unicode = pFont->UnicodeFromCharCode(elem);
      if (!unicode.IsEmpty())
        is_vaid_str = true;
      out_str += !unicode.IsEmpty() ? unicode : WideString((wchar_t)elem);
    }
    if (invalid_as_geometry && !is_vaid_str)
    {
      options.SetFlags(options.GetFlags() | RENDER_NO_NATIVETEXT | RENDER_PRINTGRAPHICTEXT);
    }
    type3_info.space_width = space_width;
    ret = ret && DrawNormalText(
      render_device, codes_storage[i], position_storage[i], pFont,
      font_size, &matrix_storage[i], fill_argb, &options, pFont->IsType3Font(), out_str, type3_info);
  }
