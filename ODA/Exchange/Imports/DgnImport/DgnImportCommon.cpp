...
void setVisibility(OdDgGraphicsElement* src, OdDbEntity* dst)
{
  ...
  if( !bInvisible )
  {
    ...
    if( !bInvisible && !pActiveView.isNull() )
    {
      ...
      if( !pActiveView->getShowPatternsFlag() && (curClass == OdDgGraphicsElement::kClassPatternComponent) )
      {
        bInvisible = true;
      }
...
void copyEntityProperties(OdDgGraphicsElement* src, OdDbEntity* dst)
{
  ...
  {
    dLineStyleScale *= getPhysicalLineStyleScaleCorrection(iLineStyleId, src, src->getLevelId());
