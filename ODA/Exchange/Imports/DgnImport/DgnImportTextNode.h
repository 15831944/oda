...
//---------------------------------------------------------------------------------------------------
  ...
  void subImportElement(OdDgElement* e, OdDbBlockTableRecord* owner) ODRX_OVERRIDE
  {
    ...
    if( textNode->getLineSpacingType() == 0 )
    {
      ...
      OdDgFontTablePtr pFontTable = textNode->database()->getFontTable();
      OdDgFontTableRecordPtr pDgFont = pFontTable->getFont(textNode->getFontEntryId());

      if( pDgFont.isNull() )
        pDgFont = pFontTable->getFont(textNode->database()->appServices()->getFAST_FONT());
      ...
      if( !pDgFont.isNull() )
      {
        ...
        if( pFont )
          dScaleFactor = 1.0 + fabs(pFont->getBelow()) / fabs(pFont->getAbove());
      }

      pMText->setLineSpacingStyle( OdDb::kExactly );

      double dLineSpacingFactor = (textNode->getLineSpacing() / dTextSize + dScaleFactor)* 3.0 / 5.0;
