         ...
         (fill && ((fillContour != _fillContour) || (fillContour && _color2 != color2))) )
    {
      ...
      if ( _fill ) 
      {
        ...
        else
        {
          ...
          _currentNode->addAttribute( svg::Stroke( _color ) );
          _currentNode->addAttribute(OD_T("stroke-width"), _bShellMode ? _currentLineWeight * 4 : _currentLineWeight); // Widen lineweight in shell mode, to mask border gaps between fills
        }
        if (_alpha != 255)
        {
  ...
  virtual void plineProc(const OdGiPolyline& lwBuf, const OdGeMatrix3d* pXform, OdUInt32 fromIndex, OdUInt32 numSegs)
  {
    ...
    if (bCreatePolyline)
    {
      ...
      if (!OdZero(dPLineWidth))
      {
        ...
        _currentNode->addAttribute(OD_T("stroke-width"), dPLineWidth*dScaleCorrection);
      }
