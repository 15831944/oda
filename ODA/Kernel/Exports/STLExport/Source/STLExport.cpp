...
namespace TD_STL_EXPORT
{
  ...
  private:
    double m_dDeviation;
  protected:
    bool m_fCorrectSolid;
    float m_epsilon;
    bool m_positiveOctant;
  public:
    StubVectorizeView() : m_dDeviation(0.), m_fCorrectSolid(false), m_epsilon(0.001f), m_positiveOctant(false) { }
    ~StubVectorizeView() { }
    ...
    void setPositiveOctant(bool positiveOctant)
    {
      m_positiveOctant = positiveOctant;
    }
  ...
  protected:
    ...
    virtual void polygonOut( OdInt32 num, const OdGePoint3d* pPnts, const OdGeVector3d* pNormal) 
    {
      ...
      if (!m_fCorrectSolid || !cell.collinear(m_epsilon))
        addTriangle(cell);
    }
  ...
  struct TryToVectorizeMod
  {
    ...
    virtual void initDevice(OdGiDrawable* pEntity, OdDbBaseDatabase *pDb, OdGsDevicePtr &pDevice, OdGiDefaultContextPtr pCtx, double dDeviation, bool fCorrectSolids, bool positiveOctant) const;
    virtual void vectorizationTest(OdGsDevicePtr pDevice) const;
  };

  void tryToVectorize(OdGiDrawable &pEntity, OdStreamBuf &pOutStream, OdDbBaseDatabase *pDb, bool bTextMode, double dDeviation, bool fCorrectSolids, bool positiveOctant, const TryToVectorizeMod &pMod = TryToVectorizeMod());

  void TryToVectorizeMod::modifyContext(OdGiDefaultContextPtr &/*pCtx*/) const { }
  
  void TryToVectorizeMod::initDevice(OdGiDrawable* pEntity, OdDbBaseDatabase *pDb, OdGsDevicePtr &pDevice, OdGiDefaultContextPtr pCtx, double dDeviation, bool fCorrectSolids, bool positiveOctant) const
  {
    ...
    ((StubVectorizeView*)pNewView.get())->setPositiveOctant(positiveOctant);
  ...
  void tryToVectorize(OdGiDrawable &pEntity, OdStreamBuf &pOutStream, OdDbBaseDatabase *pDb, bool bTextMode, double dDeviation, bool fCorrectSolids, bool positiveOctant, const TryToVectorizeMod &pMod)
  {
    odgsInitialize();
    ...
    pMod.initDevice(&pEntity, pDb, pDevice, pContext, dDeviation, fCorrectSolids, positiveOctant);
  ...
  OdResult doExport(OdDbBaseDatabase *pDb, const OdGiDrawable &pEntity, OdStreamBuf &pOutStream, bool bTextMode, double dDeviation, bool fCorrectSolid, bool positiveOctant)
  {
    OdResult ret = eOk;
    try
    {
      tryToVectorize((OdGiDrawable &)pEntity, pOutStream, pDb, bTextMode, dDeviation, fCorrectSolid, positiveOctant);
    }
  ...
  {
    return doExport(pDb, pEntity, pOutStream, bTextMode, dDeviation, false, positiveOctant);
  }
  ...
  {
    return doExport(pDb, pEntity, pOutStream, bTextMode, dDeviation, true, positiveOctant);
  }
};
