...
/** \details
  ...
  This class implements the destination geometry for two-dimensional export.
*/
...
public:
  ...
  */
  ...
  void xlineProc(
            const OdGePoint3d&, 
            const OdGePoint3d&) ODRX_OVERRIDE;
...
public:
  ...
  */
  ...
  \param bEnable [in] An option value. If its value is equal to true, the multithreaded vectorization is enabled. To switch off the option set this parameter to false.
  */
  ...
  */
  ...
  /** \details
    Is render device required to be created for specified OdGsView.
  */
...
public:
  enum //related to a view, not to a vectorizer
  {
  ...
  //Returns true if the view is used only to output hidden searchable text, the rest is
  //rendered by a render device and exported as a part of the viewport's image
  bool exportSearchableTextOnly() const;

  //Mark this view as it will be used only to output hidden searchable text, the rest is
  //rendered by a render device and exported as a part of the viewport's image
...
protected:
  ...
  */
  virtual OdGeMatrix3d metafileTransform() const { return m_mfTransformStack.metafileTransform(); };
