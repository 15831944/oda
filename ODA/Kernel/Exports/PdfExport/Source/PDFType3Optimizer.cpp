...
void PDFType3Optimizer::addUnicodeChar(const OdGiTextStyle& pTextStyle, PDFType3FontPtr pFont, OdChar pUnicode, OdAnsiString &text, OdBool isInBigFont)
{
  ...
  {
    OdChar realUnicode = pUnicode, mappedUnicode = pUnicode;
    if (isInBigFont)
    {
      OdString bigFontName = pTextStyle.bigFontFileName();
      OdCodePageId bigFontCodePage = OdCharMapper::getCpByBigFont(bigFontName);
      OdCharMapper::codepageToUnicode(realUnicode, bigFontCodePage, mappedUnicode);
    }
    ...
    if (!pElem->hasCharacter(realUnicode, singleByteCode))
    {
      ...
      if (!pElem->AddUChar(realUnicode, mappedUnicode, ShxGeom.getCharWidth(), pStream, singleByteCode))
      {
...
bool PDFType3Optimizer::checkTextScale(const OdGiTextStyle& pTextStyle, const OdString &sUnicode, const OdBoolArray &isInBigFont)
{
  ...
  OdRxObjectPtr pShxSeqHandle;
  OdRxObjectPtr pShxBigSeqHandle;
  OdRxObject* pHandle = 0;

  if (TD_PDF_HELPER_FUNCS::isShx(pOdFont))
    pShxSeqHandle = static_cast<OdShxFont*>(pOdFont)->createSequenceHandle();
  if (pBigFont && TD_PDF_HELPER_FUNCS::isShx(pBigFont))
    pShxBigSeqHandle = static_cast<OdShxFont*>(pBigFont)->createSequenceHandle();

  if (sUnicode.getLength() == 1)
  {
    if (!isInBigFont[0] && !pShxSeqHandle.isNull())
    {
      pHandle = pShxSeqHandle.get();
      if (static_cast<OdShxFont*>(pOdFont)->getLastScale(pHandle).isEqualTo(OdGePoint2d(1., 1.)))
        return true;
    }
    else if (isInBigFont[0] && !pShxBigSeqHandle.isNull())
    {
      pHandle = pShxBigSeqHandle.get();
      if (static_cast<OdShxFont*>(pBigFont)->getLastScale(pHandle).isEqualTo(OdGePoint2d(1., 1.)))
        return true;
    }
  }
  ...
  for (int f = 0; f < sUnicode.getLength(); ++f)
  {
    ...
    if (isInBigFont[f])
    {
      ODA_ASSERT(pBigFont);
      pHandle = pShxBigSeqHandle.get();
      ODA_VERIFY(static_cast<OdShxFont*>(pBigFont)->drawCharacterImp(ch, adv, &emptyGeom, textFlags, pHandle) == ::eOk);
      if (!static_cast<OdShxFont*>(pBigFont)->getLastScale(pHandle).isEqualTo(OdGePoint2d(1., 1.)))
        return false;
    }
    else
    {
      pHandle = pShxSeqHandle.get();
      ODA_VERIFY(static_cast<OdShxFont*>(pOdFont)->drawCharacterImp(ch, adv, &emptyGeom, textFlags, pHandle) == ::eOk);
      if (!static_cast<OdShxFont*>(pOdFont)->getLastScale(pHandle).isEqualTo(OdGePoint2d(1., 1.)))
        return false;
    }
  }
