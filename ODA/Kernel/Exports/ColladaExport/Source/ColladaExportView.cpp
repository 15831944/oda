...
namespace TD_COLLADA_EXPORT
{
  ...
  void OdColladaOut::addColladaEntData()
  {
    ...
    //we need either to reset iColladaEntCounter or create some meta class for the whole import
  ...
  void OdColladaOut::fillMaterialCache(MaterialData& mData, const OdGiMaterialTraitsData &materialData)
  {
    ...
    if (ambientColor.method() == OdGiMaterialColor::kOverride) {
      if (this->gsView()->mode() == OdGsView::RenderMode::kGouraudShadedWithWireframe || this->gsView()->mode() == OdGsView::RenderMode::kGouraudShaded)
    ...
    if (mData.bDiffuseChannelEnabled && diffuseMap.source() == OdGiMaterialMap::kFile && !diffuseMap.sourceFileName().isEmpty()
        &&( this->gsView()->mode() == OdGsView::RenderMode::kGouraudShadedWithWireframe || this->gsView()->mode() == OdGsView::RenderMode::kGouraudShaded))
    {
