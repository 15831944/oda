...
namespace TD_COLLADA_EXPORT
{
  ...
  */
  OdResult CollectData(OdDbBaseDatabase *pDb, EntityDataArray* m_pEntityDataArr, ColladaMaterialData* pColladaMaterialData, LightExporter* pLightExp, const ODCOLORREF* pPallete, int numColors, const OdGiDrawable* pEntity, 
  ...
  */
  OdResult exportCollada(OdDbBaseDatabase *pDb, const OdString& pFileName, const ODCOLORREF* pPallete, int numColors, const OdGiDrawable* pEntity = NULL,
    const OdGeMatrix3d& matTransform = OdGeMatrix3d::kIdentity, const std::map<OdDbStub*, double>* pMapDeviations = NULL, bool useDoublePrecision = false);
};
