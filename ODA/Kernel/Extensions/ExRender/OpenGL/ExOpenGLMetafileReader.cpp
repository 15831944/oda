      ...
      case OdOpenGLMetaRecType_DrawArrays:
        {
          ...
          if (!inprocCtx.pMarks)
          {
            if( !GETBIT(inprocCtx.bFlags, kHiddenGeometry) )
            {
          ...
          SETBIT(inprocCtx.bFlags, kHiddenGeometry, GETBIT( newState, OdOpenGLMetafileReaderCallback::kVisibilityStateChanged ) );
            ...
            do
            {
              if (inprocCtx.nMarks)
                ...
                ctDiff = count - first;
              if (ctDiff && !GETBIT(inprocCtx.bFlags, kHiddenGeometry))
              {
                if (!GETBIT(inprocCtx.bFlags, kLineweightProc | kLinesAntiAlias) || !m_pCallback)
...
#undef OD_GL_INDEX_TYPE
      ...
      case OdOpenGLMetaRecType_VPoint:
        if (!GETBIT(inprocCtx.bFlags, kSuppressGeom) && !GETBIT(inprocCtx.bFlags, kHiddenGeometry))
        {
      ...
      case OdOpenGLMetaRecType_TtfText:
        {
          if (m_pCallback && !GETBIT(inprocCtx.bFlags, kSuppressGeom) && !GETBIT(inprocCtx.bFlags, kHiddenGeometry) )
          {
