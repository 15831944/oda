...
void ExGsGDIVectorizeDevice::onSize(const OdGsDCRect& outputRect)
{
  ...
  if(m_hbmScreen) // substitute the bitmap
  {
    ...
    {
      m_hOffscreenDC.reset(new HDCStore(hdc(), getTransparencyMode() ));
...
HDC ExGsGDIVectorizeDevice::createDrawDc()
{
  ...
  {
    m_hOffscreenDC.reset(new HDCAutoDestroyStore(::CreateCompatibleDC(hdc()), getTransparencyMode() ));
    int w = width();
    int h = height();
...
OdGeMatrix3d ExGDIVectorizeView::metafileTransform() const
{
